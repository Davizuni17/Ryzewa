...
function signalStorage(auth, lidMapping, logger) {
    ...
    return {
        loadSession: async (id) => {
            try {
                ...
                // no se relanza: devolver null hace que assertSessions vuelva a pedir
                // el bundle. Pero tiene que quedar en el log, un registro corrupto aqui
                // es exactamente lo que produce "Waiting for this message"
                logger?.warn?.({ id, err: err?.message }, 'failed to load session record')
                return null
            }
            return null
        },
