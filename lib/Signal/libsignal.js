...
function isLibsignalSessionMissingError(err) {
    if (!err) return false
    const msg = err.message || String(err)
    return err.name === 'SessionError' || msg.includes('No sessions') || msg.includes('No session record')
}
...
function makeLibSignalRepository(auth, onWhatsAppFunc, logger) {
    ...
    return {
        ...
        async validateSession(jid) {
            ...
            let session = getSessionFromCache(addr)
            if (session) {
                if (typeof session.haveOpenSession === 'function' && session.haveOpenSession()) {
                    return { exists: true, cached: true }
                }
                sessionCache.delete(addrStr)
            }

            session = await storage.loadSession(addrStr)
            if (session && typeof session.haveOpenSession === 'function' && session.haveOpenSession()) {
        ...
        async decryptMessage({ jid, type, ciphertext }) {
            ...
            try {
                ...
                if (isLibsignalSessionMissingError(err)) {
                    sessionCache.delete(addrStr)
                    SESSION_STORAGE_CACHE.delete(addrStr)
                    logger?.warn({ jid, type, addr: addrStr }, 'decrypt: no usable Signal session, cleared caches')
                }
                throw err
            }
        },
        async encryptMessage({ jid, data }) {
            ...
            try {
                ...
                if (isLibsignalSessionMissingError(err)) {
                    ...
                    sessionCache.delete(addrStr)
                    SESSION_STORAGE_CACHE.delete(addrStr)
                    logger?.warn({ jid, addr: addrStr }, 'No session found for encryption, cleared stale cache')
                }
                throw err
            }
        },
