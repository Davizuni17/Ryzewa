...
async function prepareStickerPackMessage(stickerPack, options) {
    ...
    const stickerPromises = stickers.map(async (s, i) => {
        ...
        const hash = crypto_2.sha256(webpBuffer).toString('base64').replace(/\//g, '-')
    ...
    console.log('[stickerPack] ZIP upload result:', JSON.stringify(stickerPackUploadResult))
    console.log('[stickerPack] ZIP directPath:', stickerPackUploadResult.directPath, 'handle:', stickerPackUploadResult.handle)
    ...
    try {
        ...
        if ('sharp' in lib && lib.sharp) {
            ...
            // Compatible with jimp v0.x (lib.jimp IS Jimp) and v1.x (lib.jimp.Jimp)
            const JimpClass = lib.jimp.Jimp || lib.jimp
            if (!JimpClass || typeof JimpClass.read !== 'function') {
                throw new Error('Jimp loaded but .read() not available')
            }
            const jimpImage = await JimpClass.read(coverBuffer)
            // jimp v1.x: .resize({ w, h }).getBuffer(mime) returns Promise
            // jimp v0.x: .resize(w, h).getBuffer(mime, callback)
            try {
                thumbnailBuffer = await jimpImage.resize({ w: 252, h: 252 }).getBuffer('image/jpeg')
            } catch {
                thumbnailBuffer = await new Promise((resolve, reject) => {
                    jimpImage.resize(252, 252).getBuffer('image/jpeg', (err, buf) => err ? reject(err) : resolve(buf))
                })
            }
        } else {
        ...
        console.log('[stickerPack] thumb upload result:', JSON.stringify(thumbUploadResult))
        ...
        console.error('[stickerPack] THUMBNAIL FAILED:', e.message || e)
        options.logger?.warn?.(`Thumbnail generation failed: ${e}`)
    }

    // Log final proto message (reemplaza buffers con su tamaño para legibilidad)
    const logMsg = { ...stickerPackMessage }
    for (const k of Object.keys(logMsg)) {
        if (Buffer.isBuffer(logMsg[k])) logMsg[k] = `<Buffer ${logMsg[k].length}b>`
    }
    console.log('[stickerPack] FINAL proto message:', JSON.stringify(logMsg, null, 2))
