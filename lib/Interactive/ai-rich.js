...
class AIRich extends BaseBuilder {
     ...
     * - `insertAt`: índice donde insertar (por defecto al final)
     */
        ...
        if (replace != null) {
            ...
            if (insertAt != null) {
                if (!Number.isInteger(insertAt) || insertAt < 0 || insertAt > this._nodes.length) {
                    throw new InvalidTargetError(`insertAt must be an integer between 0 and ${this._nodes.length}`, { insertAt })
                }
    ...
    /**
     * Reenvía el contenido actual como edición del último mensaje enviado
     * (o del `key` indicado). Útil para actualizar el mensaje dinámicamente.
     */
    async edit(jid, key = this._lastMessageKey, options = {}) {
        if (!key?.id) {
            throw new InvalidTargetError('No message key available to edit — send() first or pass a key')
        }
