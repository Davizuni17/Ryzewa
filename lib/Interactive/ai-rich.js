 ...
 * Port CommonJS de NIXCODE MessageBuilder V4.7 — creado por Nixel
 * (contribuciones: ~ Ahmad tumbuh kembang), integrado en Ryzewa.
 * Canal: https://whatsapp.com/channel/0029VbCV1ck8fewpdNb2TY2k
 *
 * Uso:
 *   const { AIRich } = require('ryzewa')
 *   const rich = new AIRich(conn)
 *       .setTitle('Mi Bot AI')
 *       .addText('Hola, esto es *markdown* con [un link](https://example.com)', { id: 'intro' })
 *   await rich.send(jid)
 *   rich.addText('Más contenido', { insertAt: 'intro' })   // inserta DESPUÉS de 'intro'
 *   await rich.sendEdit()                                   // edita el mensaje en vivo
 */
...
// versión del builder original; se usa en el material de la firma de verificación
const VERSION = '4.7'
...
class AIRich extends BaseBuilder {
        ...
        const notif = notification
            ? {
                sessionTransparencyMetadata: {
                    disclaimerText: typeof notification === 'string' ? notification : '~ Ahmad tumbuh kembang',
                    hcaId: `hca_${Date.now()}`,
    ...
    /** Construye la edición: el protocolMessage va envuelto en botForwardedMessage (tipo 14) */
    ...
    /** Edita el último mensaje enviado (o el jid/id indicados) con el contenido actual */
    ...
    async send(jid, { bypassDownload = true, forwarded = true, notification = false, includesUnifiedResponse = true, includesSubmessages = true, messageId, additionalNodes = [], ...options } = {}) {
        ...
        // auto-edición inmediata: hace que la media se renderice sin botón de descarga
    ...
    static tokenizer(code, lang = 'javascript') {
        const keywordsMap = {
            javascript: new Set([
                'break', 'case', 'catch', 'continue', 'debugger', 'delete', 'do', 'else', 'finally', 'for', 'function', 'if', 'in',
                'instanceof', 'new', 'return', 'switch', 'this', 'throw', 'try', 'typeof', 'var', 'void', 'while', 'with', 'true',
                'false', 'null', 'undefined', 'class', 'const', 'let', 'super', 'extends', 'export', 'import', 'yield', 'static',
                'constructor', 'async', 'await', 'get', 'set'
            ]),

            typescript: new Set([
                'abstract', 'any', 'as', 'asserts', 'bigint', 'boolean', 'declare', 'enum', 'implements', 'infer', 'interface', 'is',
                'keyof', 'module', 'namespace', 'never', 'readonly', 'require', 'number', 'object', 'override', 'private', 'protected',
                'public', 'satisfies', 'string', 'symbol', 'type', 'unknown', 'using', 'from', 'break', 'case', 'catch', 'continue',
                'do', 'else', 'finally', 'for', 'function', 'if', 'new', 'return', 'switch', 'this', 'throw', 'try', 'var', 'void',
                'while', 'class', 'const', 'let', 'extends', 'import', 'export', 'async', 'await'
            ]),
            ...
            cpp: new Set([
                'alignas', 'alignof', 'and', 'auto', 'bool', 'break', 'case', 'catch', 'class', 'const', 'constexpr', 'continue',
                'delete', 'do', 'double', 'else', 'enum', 'explicit', 'export', 'extern', 'false', 'float', 'for', 'friend', 'if',
            ...
            php: new Set([
                ...
                'implements', 'include', 'include_once', 'instanceof', 'interface', 'match', 'namespace', 'new', 'null', 'or',
                'private', 'protected', 'public', 'require', 'require_once', 'return', 'static', 'switch', 'throw', 'trait', 'try',
                'use', 'var', 'while', 'yield'
            ]),
            ...
            html: new Set([
                'html', 'head', 'body', 'div', 'span', 'p', 'a', 'img', 'video', 'audio', 'script', 'style', 'link', 'meta', 'form',
                'input', 'button', 'table', 'tr', 'td', 'th', 'ul', 'ol', 'li', 'section', 'article', 'header', 'footer', 'nav', 'main'
            ]),

            bash: new Set([
                'if', 'then', 'else', 'elif', 'fi', 'for', 'while', 'do', 'done', 'case', 'esac', 'function', 'in', 'select',
                'until', 'break', 'continue', 'return', 'export', 'readonly', 'local', 'declare'
            ]),
    ...
    _addContent(section, submessage, { id, replace, insertAt } = {}) {
        ...
        if (hasInsertAt) {
            ...
            // offset >= 0 inserta DESPUÉS del nodo objetivo; offset < 0, antes
    ...
    /** Inserta una sección cruda (p.ej. AIRich.newLayout('HScroll', otroRich.items)) */
    addSection(section, options = {}) {
        return this._addContent(section, undefined, options)
    }

    /** Inserta un submessage crudo */
    ...
    /** Elimina un nodo por id (o [id, offset]) */
    delete(target) {
    ...
    /** Primitivos sueltos de todas las secciones (para mezclar en otro AIRich) */
    get items() {
