...
const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        if (force) {
            ...
            for (const jid of jids) {
                ...
                const raw = sessions[signalId];
                let needsFetch = !raw;
                if (!needsFetch) {
                    try {
                        const record = libsignal.SessionRecord.deserialize(raw);
    ...
    const createParticipantNodes = async (jids, message, extraAttrs) => {
        ...
        const nodes = await Promise.all(patched.map(async (patchedMessageWithJid) => {
            ...
            let encResult;
            const isSessionMissingErr = (e) => {
                if (!e) return false
                const msg = e.message || String(e)
                return e.name === 'SessionError' || msg.includes('No sessions') || msg.includes('No session record')
            }
            try {
                ...
                logger.warn({ jid }, 'SessionError during encryption, re-asserting session and retrying')
                try {
                    await assertSessions([jid], true);
                    encResult = await signalRepository.encryptMessage({ jid, data: bytes });
                } catch (retryErr) {
                    if (!isSessionMissingErr(retryErr)) {
                        logger.error({ jid, error: retryErr.message }, 'Failed to encrypt after session re-assertion')
                        throw retryErr;
                    }
                    logger.warn({ jid }, 'SessionError persists, deleting stored session and re-fetching keys')
