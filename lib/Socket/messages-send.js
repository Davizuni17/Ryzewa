...
const makeMessagesSocket = (config) => {
    ...
    _api.sendPrivateToGroup = async (jid, content, targetJid, options = {}) => {
        ...
        // addressing mode del grupo (lid por defecto)
        let addressingMode = 'lid'
        try {
            const meta = (global.groupMetadataCache ? await global.groupMetadataCache(jid) : null) || await groupMetadata(jid)
            addressingMode = meta?.addressingMode || 'lid'
        } catch {}

        const identity = (addressingMode === 'lid' && meLid) ? meLid : meId
        const decoded = WABinary_1.jidDecode(identity)
        const signalUser = decoded?.domainType && decoded.domainType !== 0 ? `${decoded.user}_${decoded.domainType}` : decoded?.user
        const deviceId = decoded?.device || 0

        // nombre de la sender key del bot para este grupo: grupo::usuario::device
        const senderKeyName = `${jid}::${signalUser}::${deviceId}`

        // rotar: borrar la clave actual (se genera una nueva al cifrar) y la memoria de distribución
        ...
        return await _api.sendMessage(jid, content, { ...options, privateParticipant: targetJid })
    }
