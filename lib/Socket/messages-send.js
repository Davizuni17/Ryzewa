...
const makeMessagesSocket = (config) => {
    ...
    /**
     * Envía un mensaje a un GRUPO que en la práctica solo puede leer `targetJid`.
     *
     * Rota (borra) la sender key del bot para ese grupo y limpia la memoria de
     * distribución; luego reparte la clave nueva SOLO al target vía
     * privateParticipant. Los demás miembros se quedan con la clave vieja y ven
     * "Waiting for this message" (no pueden descifrar el nuevo skmsg).
     *
     * IMPORTANTE:
     * - Tu propia cuenta (el bot) siempre verá el mensaje: es el remitente.
     * - Tras esto, el PRÓXIMO mensaje normal al grupo vuelve a repartir la clave
     *   nueva a todos, restaurando el envío normal.
     *
     *   await conn.sendPrivateToGroup(grupoJid, { text: 'solo para ti' }, '504xxxx@s.whatsapp.net')
     */
    _api.sendPrivateToGroup = async (jid, content, targetJid, options = {}) => {
        if (!WABinary_1.isJidGroup(jid)) {
            throw new Error('sendPrivateToGroup solo funciona en grupos (@g.us)')
        }
        if (!targetJid) {
            throw new Error('targetJid es requerido')
        }

        const meId = authState.creds.me.id
        const meLid = authState.creds.me?.lid

        // Se rota la clave de AMBAS identidades (PN y LID): segun el addressing
        // mode del grupo el bot cifra con una u otra, y borrar solo una dejaria
        // la clave vieja viva (todos seguirian descifrando).
        const identities = [meId, meLid].filter(Boolean)
        const rotated = []
        ...
        // limpiar a quien se le habia distribuido la clave
        ...
        // enviar repartiendo la clave nueva solo al target
        const msg = await _api.sendMessage(jid, content, { ...options, privateParticipant: targetJid })

        msg.privateDebug = { target: targetJid, rotated }
