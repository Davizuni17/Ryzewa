...
const makeMessagesSocket = (config) => {
    ...
    _api.rememberLastMessageId = (chatJid, participantJid, messageId) => {
        if (!chatJid || !participantJid || !messageId) return

        if (lastMessageIds.size >= LAST_IDS_MAX) {
            // el Map conserva orden de insercion: se descarta lo mas antiguo
            const oldest = lastMessageIds.keys().next().value
            lastMessageIds.delete(oldest)
        }

        lastMessageIds.set(`${chatJid}|${WABinary_1.jidNormalizedUser(participantJid)}`, messageId)
    }

    _api.getLastMessageId = (chatJid, participantJid) =>
        lastMessageIds.get(`${chatJid}|${WABinary_1.jidNormalizedUser(participantJid)}`)
