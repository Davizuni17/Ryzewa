            ...
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content && typeof content['disappearingMessagesInChat'] !== 'undefined' && WABinary_1.isJidGroup(jid)) {
                ...
                const fullMsg = await Utils_1.generateWAMessage(jid, content, {
                    ...
                    upload: async (encFilePath, opts) => {
                        ...
                        // Use handle from server, or fallback to fileEncSha256B64 for
                        // endpoints that don't return handle (e.g. /mms/sticker-pack)
                        mediaHandle = up.handle || opts.fileEncSha256B64
                        console.log('[sendMessage upload] handle:', up.handle, 'fallback:', opts.fileEncSha256B64, 'using:', mediaHandle)
                        return up
                    },
                    mediaCache: config.mediaCache,
                    options: config.options,
                ...
                console.log('[sendMessage] mediaHandle:', mediaHandle)
                if (mediaHandle) {
                    additionalAttributes['media_id'] = mediaHandle
                }
                console.log('[sendMessage] additionalAttributes:', JSON.stringify(additionalAttributes))
