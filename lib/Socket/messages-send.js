...
const makeMessagesSocket = (config) => {
     ...
     *
     * `target` acepta el mensaje completo (m), su key, o directamente el id:
     *
     *   await conn.sendPrivateToGroup(m.chat, { text: 'solo para ti' }, m)
     *
     * Requiere un mensaje previo de esa persona; sin él no hay id que reutilizar.
     */
    _api.sendPrivateToGroup = async (jid, content, target, options = {}) => {
        const messageId =
            typeof target === 'string'
                ? target
                : target?.key?.id || target?.id?.id || target?.id || options.messageId

        if (!messageId || typeof messageId !== 'string' || messageId.includes('@')) {
            throw new Error('sendPrivateToGroup necesita el mensaje de la persona que debe verlo (pasa `m`), no su jid')
        }
    ...
    _api.relayPrivateToGroup = async (jid, message, target, options = {}) => {
        ...
        if (!messageId || typeof messageId !== 'string' || messageId.includes('@')) {
            throw new Error('relayPrivateToGroup necesita el mensaje de la persona que debe verlo (pasa `m`), no su jid')
        }
