...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, code) => {
        // a hardcoded default would give every bot on this library the same code,
        // which is trivially fingerprintable — fall back to a random one
        const pairingCode = code
            ? code.toUpperCase().replace(/[^A-Z0-9]/g, '').substring(0, 8).padEnd(8, 'X')
            : Utils_1.bytesToCrockford(crypto_1.randomBytes(5))
