...
const makeSocket = (config) => {
    ...
    const uploadPreKeys = async (count = Defaults_1.INITIAL_PREKEY_COUNT, retryCount = 0) => {
        ...
        const uploadLogic = async (retryCount) => {
            ...
            const { update, node } = await keys.transaction(async () => {
                logger.debug({
                    requestedCount: count
                }, 'generating pre-keys with requested count')
                return await Utils_1.getNextPreKeysNode({
                    creds,
                    keys
                }, count)
            ...
            try {
                await query(node)
                // el update marca las prekeys como "ya subidas". Si se emite antes del
                // query y este falla, quedan marcadas como subidas sin que el servidor
                // las tenga -> hueco permanente y nadie puede abrir sesion con el bot
    ...
    const uploadPreKeysToServerIfRequired = async () => {
        try {
            ...
            // Se repone SIEMPRE con el lote completo. Reponer de 5 en 5 deja al bot sin
            // pre-keys en el servidor en segundos (cada device nuevo que abre sesion
            // consume una) y entonces nadie puede establecer sesion -> "Waiting for
            // this message". No hay riesgo de rate-limit porque solo se sube cuando el
            // contador del servidor baja del umbral (MIN_PREKEY_COUNT), no en cada
            // reconexion.
            const count = Defaults_1.INITIAL_PREKEY_COUNT
            const {
