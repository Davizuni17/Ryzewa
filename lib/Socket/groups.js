...
const makeGroupsSocket = (config) => {
    ...
    const groupQuery = async (jid, type, content, extraAttrs) => (query({
        tag: 'iq',
        attrs: {
            type,
            xmlns: 'w:g2',
    ...
    /**
     * Cuando se direccionan participantes por LID, el IQ debe declararlo.
     * Sin `addressing_mode: 'lid'` el servidor responde internal-server-error (500),
     * aunque el LID enviado sea exactamente el que el propio servidor usa en sus
     * notificaciones `w:gp2` de ese grupo.
     */
    const lidAddressingAttrs = (participants) => {
        const list = Array.isArray(participants) ? participants : [participants]
        const usesLid = list.some(p => typeof p === 'string' && p.endsWith('@lid'))
        return usesLid ? { addressing_mode: 'lid' } : undefined
    }
    ...
    return {
        ...
        groupCreate: async (subject, participants) => {
            ...
            ], lidAddressingAttrs(participants))
        ...
        groupRevokeInviteV4: async (groupJid, invitedJid) => {
            const result = await groupQuery(groupJid, 'set', [{ tag: 'revoke', attrs: {}, content: [{ tag: 'participant', attrs: { jid: invitedJid } }] }], lidAddressingAttrs(invitedJid))
