...
const makeMessagesRecvSocket = (config) => {
    ...
    const sendMessagesAgain = async (key, ids, retryNode) => {
        ...
        // Los mensajes enviados con privateParticipant solo puede leerlos el
        // target: atender el reintento de otro le entregaria la sender key.
        // Se comprueba por id dentro del bucle para no desalinear ids/msgs.
        const isBlockedPrivate = id => !!sock.isBlockedPrivateRetry?.(id, participant)

        if (ids.every(isBlockedPrivate)) {
            logger.debug({ participant, ids }, 'ignorando retry de mensaje privado de grupo')
            return
        }
        ...
        for (const [i, msg] of msgs.entries()) {
            ...
            if (isBlockedPrivate(ids[i])) {
                logger.debug({ participant, id: ids[i] }, 'retry ignorado: mensaje privado de grupo')
                continue
            }
        ...
        try {
            await Promise.all([
                processingMutex.mutex(msg?.key?.remoteJid || node.attrs.from || '__msg__', async () => {
                    ...
                    // recuerda el ultimo mensaje de cada participante: sendPrivateToGroup
                    // reutiliza ese id para dirigir un mensaje solo a esa persona
                    if (!msg.key.fromMe && msg.key.id && msg.key.remoteJid) {
                        sock.rememberLastMessageId?.(
                            msg.key.remoteJid,
                            msg.key.participant || msg.key.remoteJid,
                            msg.key.id,
                            Utils_1.toNumber(msg.messageTimestamp) || Math.floor(Date.now() / 1000)
                        )
                    }
    ...
    // El historial que manda WhatsApp al conectar tambien sirve para recordar
    // el ultimo mensaje de cada participante, asi sendPrivateToGroup puede
    // dirigirse a alguien que no ha escrito desde que arranco el bot.
    ev.on('messaging-history.set', ({ messages }) => {
        if (!Array.isArray(messages) || !sock.rememberLastMessageId) return

        for (const msg of messages) {
            if (msg?.key?.fromMe || !msg?.key?.id || !msg?.key?.remoteJid) continue

            sock.rememberLastMessageId(
                msg.key.remoteJid,
                msg.key.participant || msg.key.remoteJid,
                msg.key.id,
                Utils_1.toNumber(msg.messageTimestamp) || 0
            )
        }
    })

    /**
     * Pide a WhatsApp historial de un chat para llenar el cache de targets
     * privados sin esperar a que la gente escriba. La respuesta llega de forma
     * asincrona por messaging-history.set, asi que conviene esperar unos
     * segundos antes de consultar getLastMessageId.
     *
     *   await conn.loadChatTargets(m.chat, m, 50)
     */
    const loadChatTargets = async (jid, refMsg, count = 50) => {
        const key = refMsg?.key || refMsg

        if (!key?.id) {
            throw new Error('loadChatTargets necesita un mensaje de referencia de ese chat (pasa `m`)')
        }

        const timestampMs = Utils_1.toNumber(refMsg?.messageTimestamp || key.messageTimestamp || 0) * 1000 || Date.now()

        await fetchMessageHistory(count, { ...key, remoteJid: jid }, timestampMs)

        return { requested: count, chat: jid }
    }
